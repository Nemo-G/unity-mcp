...
- **Control Window**: Full-featured TCP bridge management interface (`Tools > Unity TCP Bridge > Control Window`)
- **Status Window**: Compact dockable status monitor (`Tools > Unity TCP Bridge > Status Window`)
- **Menu Items**: Quick actions via Unity menu (`Tools > Unity TCP Bridge`)
...
1. **Control Window**: `Tools > Unity TCP Bridge > Control Window`
...
```
External TCP Client → Unity TCP Server → Custom Protocol Handler
```
...
3. The TCP server will start automatically
4. Check Unity Console for server status
