...
namespace UnityTcp.Editor
{
    ...
    public static partial class UnityTcpBridge
    {
        ...
        public static void Start()
        {
            lock (startStopLock)
            {
                ...
                if (isRunning && listener != null)
                {
                    if (IsDebugEnabled())
                    {
                        Debug.Log($"<b><color=#2EA3FF>UNITY-TCP</color></b>: UnityTcpBridge already running on port {currentUnityPort}");
                    }
                    return;
                }
                ...
                try
                {
                    ...
                    Debug.Log($"<b><color=#2EA3FF>UNITY-TCP</color></b>: UnityTcpBridge started on port {currentUnityPort}. (OS={platform}, server={serverVer})");
        ...
        public static void Stop()
        {
            ...
            if (IsDebugEnabled()) Debug.Log("<b><color=#2EA3FF>UNITY-TCP</color></b>: UnityTcpBridge stopped.");
        }
                ...
                try
                {
                    ...
                    try
                    {
                        if (IsDebugEnabled())
                        {
                            ...
                            Debug.Log($"<b><color=#2EA3FF>UNITY-TCP</color></b>: Client connected {ep}");
                        }
                    }
                    catch { }
