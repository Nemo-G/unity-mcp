    ...
    public static class ManageScript
    {
        ...
        public static object HandleCommand(JObject @params)
        {
            ...
            switch (action)
            {
                ...
                case "read":
                    TcpLog.Warn("manage_script.read is deprecated; prefer resources/read. Serving read for backward compatibility.");
                    return ReadScript(fullPath, relativePath);
                case "update":
                    TcpLog.Warn("manage_script.update is deprecated; prefer apply_text_edits. Serving update for backward compatibility.");
            ...
            string validateMode = null)
        {
            ...
            try
            {
                ...
                if (immediate)
                {
                    TcpLog.Info($"[ManageScript] ApplyTextEdits: immediate refresh for '{relativePath}'");
                ...
                {
                    TcpLog.Info($"[ManageScript] ApplyTextEdits: debounced refresh scheduled for '{relativePath}'");
                    ManageScriptRefreshHelpers.ScheduleScriptRefresh(relativePath);
                }
            ...
            JObject options)
        {
            ...
            try
            {
                ...
                if (immediate)
                {
                    TcpLog.Info($"[ManageScript] EditScript: immediate refresh for '{relativePath}'", always: false);
                    ManageScriptRefreshHelpers.ImportAndRequestCompile(relativePath);
                }
                else
                {
